<x-admin>
   ...
   <form class="form-contact" action="{{route('admin.system.store')}}" method="post" enctype="multipart/form-data">
    ...
    <div class="grid grid-col-2 gap-4">
        <div>
            <label for="name">{{__('locker.Add New Locker')}}</label>
        ...
        <div>
            ...
            @if ($places->isEmpty())
             <a class="text-blue-700" href="{{route('admin.place.create')}}">{{__('locker.No available places to Create new Orte from.')}} &#x2192;</a>
            @else
    ...
    <button type="submit" class="text-white bg-indigo-500 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-blue-800">{{__('place.Create')}}</button>
    </div>
